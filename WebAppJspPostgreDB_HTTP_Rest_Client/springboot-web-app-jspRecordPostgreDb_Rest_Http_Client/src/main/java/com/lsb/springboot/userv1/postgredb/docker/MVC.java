...
{	
	...
	public MVC(UserList userL)
	{
	 ...
	 @PutMapping("/update/name") void updateM(@Valid @RequestBody UserRecordV1 user)
	 { 
		 userL.save(user);
	 }
	...
	//post
	 ...
	 @PostMapping("") void Create(@Valid @RequestBody UserRecordV1 user) 
	 {
		 userL.save(user);
	 }
	 ...
	 List<UserRecordV1> findBylocation(@PathVariable String name)
	 {
		 return userL.findAllByName(name);
	 }
	 
	 @GetMapping("")
	 List<UserRecordV1> findAllM() 
	 { 
		 return userL.findAll(); 
	 }
	 
	 @GetMapping("/{id}") UserRecordV1 findByIdM(@PathVariable int id) { Optional<UserRecordV1>
